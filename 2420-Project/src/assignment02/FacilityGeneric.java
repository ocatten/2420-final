...
public class FacilityGeneric<Type> {
	...
	public CurrentPatientGeneric<Type> lookupByUHID (UHealthID patientID) {
		...
		// Check each generic patient in the patientList for a match
		for (CurrentPatientGeneric<Type> currentPatient : patientList) {
	...
	public ArrayList< CurrentPatientGeneric<Type> > lookupByPhysician (Type physician) {
		...
		for ( CurrentPatientGeneric<Type> currentPatient : patientList) {
			if (currentPatient.getPhysician().equals(physician)) {
				...
				physicianPatients.add(currentPatient); // Add a patient with a matching physician to the list
			}
		}
	 ...
	 * @param physician - generic identifier of patient's new physician
	 */
	...
	public void addAll(String filename) {
		try {
			...
			while (fileIn.hasNextLine()) {
				...
				lineNumber++;
				 CurrentPatientGeneric<Type> patient = parsePatient(line, lineNumber);
	...
	private  CurrentPatientGeneric<Type> parsePatient(String line, int lineNumber) throws ParseException {
		...
		return new  CurrentPatientGeneric<Type>(firstName, lastName, new UHealthID(patientIDString), 
								physician, lastVisit);
	}*/
}
